...
class ResidualBlock(nn.Module):
    ...
    def forward(self,x):
        ...
        ans1 = x + self.Layer2(self.Linear(self.Layer1(x)))
...
class PINN:
    ...
    def eq_loss(self, X: torch.Tensor):
        ...
        self.ls_f1 = self.loss(R1,torch.zeros_like(R1))  #方程1损失函数
    ...
    def closure(self) -> torch.nn:
        ...
        if self.ns > 0 and self.nf > 0:
            self.ls = self.ls_s + 0.5*self.ls_f     #PDE方程损失函数与预测精确的和
        elif self.ns > 0:
